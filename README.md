...
**当前为模拟器验证版本：四个工程已通过模拟器构建，三款新 App 的界面、姿态切换与核心交互已在 iPhone Duo 模拟器中实际操作确认；真机、音频听感与辅助功能尚未验证。** 验证范围见 [记录](Docs/VALIDATION.md)。每个 Demo 只关注一组交互，可以单独打开、编译和修改，无需账号、服务器或第三方依赖。
